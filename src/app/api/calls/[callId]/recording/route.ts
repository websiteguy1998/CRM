 ...
 * the URL under). Only full-visibility roles can play these back, since
 * there's no lead ownership to check against.
 */
export async function GET(_req: NextRequest, { params }: { params: Promise<{ callId: string }> }) {
  const auth = await requireApiSession();
  if ("error" in auth) return auth.error;
  if (!hasFullLeadVisibility(auth.session.role)) {
